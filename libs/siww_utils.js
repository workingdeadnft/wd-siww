...
export const checkIsValidStatement = (_str) => {
  ...
  if (!_isValid) {
    console.log("invalid message: " + _str);
  }
  return _isValid;
};
...
export const checkIsValidChain = (_chain, aAcceptedChains) => {
  if (!aAcceptedChains || !_chain) {
    return false;
  }
  let _isValid = aAcceptedChains.some(
    (item) => item.name.toLowerCase() === _chain.toLowerCase()
  );
  if (!_isValid) {
    console.log("invalid chain: " + _chain);
  }
  return _isValid;
};
